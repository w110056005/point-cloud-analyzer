...
from DE import solve_de_transf
from scipy.spatial.transform import Rotation
from PrintResult import find_new_file
import csv
...
def get_init_matches(feature_source, feature_target):
    ...
    ##建立 pairs
    _,N = feature_source.shape # N : fpfh feature數量
    matches = []
    for i in range(N):
        query = feature_source[:,i]
        _, idx_nn_target, _ = fpfh_search_tree.search_knn_vector_xd(query, 1)   #source -> target
        matches.append([i,idx_nn_target[0]])    #通過 knn 尋找唯一 的 nearest points 建立對應點
...
def iter_match(
    ...
    T = solve_de_transf(points_source, points_target)    #透過 svd 初步求解 旋转、平移矩阵
    ...
    #判断数目
    is_valid_correspondence_distance = np.all(deviation <= checker_params.max_correspondence_distance)#icp
...
def ransac_match(
    ...
    for i in range(ransac_params.max_iteration):
        ...
        T = DE(idx_target,idx_source,pcd_source, pcd_target, matches, checker_params)
...
def DE(idx_target,idx_source,
    ...
    # get corresponding points:
    points_source = np.asarray(pcd_source.points)[idx_source]
    points_target = np.asarray(pcd_target.points)[idx_target] ## 按照對應的source 排序

    T = solve_de_transf(points_source, points_target)  # 透過 de 初步求解 旋转、平移矩阵
    #xxxxx()
    R, t = T[0:3, 0:3], T[0:3, 3]
    # deviation：偏差  区分 inline outline 通过 距离判断
    deviation = np.linalg.norm(
    ...
    return T
def DE_match(idx_target,idx_source,
        pcd_source, pcd_target,
        feature_source, feature_target,
        ransac_params, checker_params
):
    # step5.1 利用fpfh 建立 cprresponding pairs
    matches = get_init_matches(feature_source, feature_target) # matches [target_id, source_id]
    ## build search tree on the target:
    search_tree_target = o3d.geometry.KDTreeFlann(pcd_target)
    N, _ = matches.shape
    idx_matches = np.arange(N)
    ...
    # ##step5.3 iter 迭代， iter_match() ,选择出 vaild T
    # validator = lambda proposal: iter_match(idx_target,idx_source,pcd_source, pcd_target, proposal, checker_params)
    # print("Multi Thread Calculating.....")
    # with concurrent.futures.ThreadPoolExecutor(max_workers=ransac_params.max_workers) as executor:
    #     for T in map(
    #             validator,
    #             proposal_generator        #map()是 Python 内置的高阶函数，它接收一个函数 f 和一个 list，并通过把函数 f 依次作用在 list 的每个元素上，得到一个新的 list 并返回。
